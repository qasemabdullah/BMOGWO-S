...
clc
%*************************Initialization********************************************%
data = LoadData()   %load the data
nVar=data.nx;       % Number of Decision Variables
VarSize=[1 nVar];   % Size of Decision Variables Matrix
VarMin=0;         % Lower Bound of Variables
VarMax=1;         % Upper Bound of Variables
CostFunction=@(u) FeatureSelectionCost(u,data); %Objective Function 
...
%*************** Binarize the Positions of search agents******************
        sig_pos = [];
        xx=sigmf((X1+X2+X3)/3,[10 0.5]);
        for eachpos = xx  
            if eachpos< 0.5
                bpos=0;
            else
            ...
            sig_pos = [sig_pos bpos];
        end
            GreyWolves(i).Position=sig_pos;
%**************************************************************************
        % Boundary checking
        GreyWolves(i).Position=(min(max(GreyWolves(i).Position,lb),ub));
         if numel(find(GreyWolves(i).Position ~= 0)) == 0
            continue;
        end
...
disp('Time')
disp(time);
%************ Save the obtained non-dominated soultions (SF,ER)************
S.('BMOGWO_Zoo') = Archive_costs;
save('Zoo/BMOGWOs_zoo1.mat', '-struct', 'S')  %each run you have to change the number
%**************************************************************************
%****************Final Results*********************************************
figure 
load('ZOO\BMOGWOs_zoo1.mat')
BMOGWO_Zoo= transpose(BMOGWO_Zoo);
BMOGWO_Zoo= sortrows(BMOGWO_Zoo, 1);
plot(BMOGWO_Zoo(:, 1),BMOGWO_Zoo(:, 2), '-.b*',...
    'LineWidth',1.5,...
    'MarkerEdgeColor','b',...
